...
### `bedrock-agent` calls go via raw SigV4 rather than typed boto3

The tool calls bedrock-agent (control plane + runtime) with a SigV4-signing
client and hand-built JSON instead of the typed boto3 client. The two pieces
it depends on — the `MANAGED_KNOWLEDGE_BASE_CONNECTOR` envelope and
`Retrieve.userContext` — are available in the typed client as of boto3
1.43.32, so this is an implementation choice rather than an SDK gap. The
signing client is what has been validated end-to-end, which is why it ships.

A future release will retire it in favour of typed boto3 calls, raising the
floor to `boto3 >= 1.43.32` and adding curated TOML fields for the typed
knobs that come with it (deletion protection, media extraction). The
override table (`connector_params_overrides`) stays either way, since
`connectorParameters` is a free-form structure and any field the tool
doesn't model passes through unchanged.
...
### Idempotency tokens on Create* writes

`StartIngestionJob` retries are idempotency-aware (the SDK provides a
client request token), but `CreateKnowledgeBase` and `CreateDataSource`
retry only on connection errors and throttling. A retry across a real
network partition during one of those calls could in principle create a
duplicate resource, though the per-run unique resource naming prevents
that in practice.
